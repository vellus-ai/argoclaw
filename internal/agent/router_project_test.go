...
func TestGetForProject_EmptyProjectFallsBackToGet(t *testing.T) {
	r := NewRouter()
	r.SetResolver(func(ctx context.Context, agentKey string, opts ResolveOpts) (Agent, error) {
		return &mockAgent{id: agentKey}, nil
	})

	ctx := context.Background()
	ag, err := r.GetForProject(ctx, "sdlc-assistant", "", nil)
	if err != nil {
...
func TestGetForProject_DifferentProjectsSeparateCache(t *testing.T) {
	...
	ctx := context.Background()
	ag1, err := r.GetForProject(ctx, "sdlc-assistant", "uuid-xpos", nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	ag2, err := r.GetForProject(ctx, "sdlc-assistant", "uuid-payment", nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestGetForProject_SameProjectUsesCache(t *testing.T) {
	...
	ctx := context.Background()
	_, _ = r.GetForProject(ctx, "sdlc-assistant", "uuid-xpos", nil)
	_, _ = r.GetForProject(ctx, "sdlc-assistant", "uuid-xpos", nil)
...
func TestGetForProject_NoProjectAndWithProject_SeparateCache(t *testing.T) {
	...
	ctx := context.Background()
	ag1, _ := r.GetForProject(ctx, "sdlc-assistant", "", nil)
	ag2, _ := r.GetForProject(ctx, "sdlc-assistant", "uuid-xpos", nil)
