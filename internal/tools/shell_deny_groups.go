...
var DenyGroupRegistry = map[string]*DenyGroup{
	...
	"env_dump": {
		...
		Patterns: []*regexp.Regexp{
			...
			regexp.MustCompile(`(?i)\becho\b.*\$\{?ARGOCLAW_(GATEWAY_TOKEN|ENCRYPTION_KEY|POSTGRES_DSN)`),
			regexp.MustCompile(`(?i)\bprintf\b.*\$\{?ARGOCLAW_(GATEWAY_TOKEN|ENCRYPTION_KEY|POSTGRES_DSN)`),
			regexp.MustCompile(`\bpython[23]?\b.*os\.(environ|getenv).*ARGOCLAW_`),
			regexp.MustCompile(`\bnode\b.*-e.*process\.env\.ARGOCLAW_`),
		},
	},
}
