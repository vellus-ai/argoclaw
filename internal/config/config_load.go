...
func Default() *Config {
	return &Config{
		DataDir: "~/.argoclaw/data",
		Agents: AgentsConfig{
			Defaults: AgentDefaults{
				Workspace:           "~/.argoclaw/workspace",
...
func (c *Config) applyEnvOverrides() {
	...
	envStr("ARGOCLAW_ANTHROPIC_API_KEY", &c.Providers.Anthropic.APIKey)
	envStr("ARGOCLAW_ANTHROPIC_BASE_URL", &c.Providers.Anthropic.APIBase)
	envStr("ARGOCLAW_OPENAI_API_KEY", &c.Providers.OpenAI.APIKey)
	envStr("ARGOCLAW_OPENAI_BASE_URL", &c.Providers.OpenAI.APIBase)
	envStr("ARGOCLAW_OPENROUTER_API_KEY", &c.Providers.OpenRouter.APIKey)
	envStr("ARGOCLAW_GROQ_API_KEY", &c.Providers.Groq.APIKey)
	envStr("ARGOCLAW_DEEPSEEK_API_KEY", &c.Providers.DeepSeek.APIKey)
	envStr("ARGOCLAW_GEMINI_API_KEY", &c.Providers.Gemini.APIKey)
	envStr("ARGOCLAW_MISTRAL_API_KEY", &c.Providers.Mistral.APIKey)
	envStr("ARGOCLAW_XAI_API_KEY", &c.Providers.XAI.APIKey)
	envStr("ARGOCLAW_MINIMAX_API_KEY", &c.Providers.MiniMax.APIKey)
	envStr("ARGOCLAW_COHERE_API_KEY", &c.Providers.Cohere.APIKey)
	envStr("ARGOCLAW_PERPLEXITY_API_KEY", &c.Providers.Perplexity.APIKey)
	envStr("ARGOCLAW_DASHSCOPE_API_KEY", &c.Providers.DashScope.APIKey)
	envStr("ARGOCLAW_BAILIAN_API_KEY", &c.Providers.Bailian.APIKey)
	envStr("ARGOCLAW_ZAI_API_KEY", &c.Providers.Zai.APIKey)
	envStr("ARGOCLAW_ZAI_CODING_API_KEY", &c.Providers.ZaiCoding.APIKey)
	envStr("ARGOCLAW_OLLAMA_HOST", &c.Providers.Ollama.Host)
	...
	envStr("ARGOCLAW_LARK_APP_ID", &c.Channels.Feishu.AppID)
	envStr("ARGOCLAW_LARK_APP_SECRET", &c.Channels.Feishu.AppSecret)
	envStr("ARGOCLAW_LARK_ENCRYPT_KEY", &c.Channels.Feishu.EncryptKey)
	envStr("ARGOCLAW_LARK_VERIFICATION_TOKEN", &c.Channels.Feishu.VerificationToken)
	envStr("ARGOCLAW_WHATSAPP_BRIDGE_URL", &c.Channels.WhatsApp.BridgeURL)
	envStr("ARGOCLAW_SLACK_BOT_TOKEN", &c.Channels.Slack.BotToken)
	envStr("ARGOCLAW_SLACK_APP_TOKEN", &c.Channels.Slack.AppToken)
	envStr("ARGOCLAW_SLACK_USER_TOKEN", &c.Channels.Slack.UserToken)

	// TTS secrets
	envStr("ARGOCLAW_TTS_OPENAI_API_KEY", &c.Tts.OpenAI.APIKey)
	envStr("ARGOCLAW_TTS_ELEVENLABS_API_KEY", &c.Tts.ElevenLabs.APIKey)
	envStr("ARGOCLAW_TTS_MINIMAX_API_KEY", &c.Tts.MiniMax.APIKey)
	envStr("ARGOCLAW_TTS_MINIMAX_GROUP_ID", &c.Tts.MiniMax.GroupID)
	...
	// Claude CLI provider
	envStr("ARGOCLAW_CLAUDE_CLI_PATH", &c.Providers.ClaudeCLI.CLIPath)
	envStr("ARGOCLAW_CLAUDE_CLI_MODEL", &c.Providers.ClaudeCLI.Model)
	envStr("ARGOCLAW_CLAUDE_CLI_WORK_DIR", &c.Providers.ClaudeCLI.BaseWorkDir)
	...
	envFallback("ARGOCLAW_PROVIDER", &c.Agents.Defaults.Provider)
	envFallback("ARGOCLAW_MODEL", &c.Agents.Defaults.Model)

	// Data directory, workspace & sessions
	envStr("ARGOCLAW_DATA_DIR", &c.DataDir)
	envStr("ARGOCLAW_WORKSPACE", &c.Agents.Defaults.Workspace)

	// Gateway host/port
	envStr("ARGOCLAW_HOST", &c.Gateway.Host)
	if v := os.Getenv("ARGOCLAW_PORT"); v != "" {
	...
	// Database
	envStr("ARGOCLAW_POSTGRES_DSN", &c.Database.PostgresDSN)
	envStr("ARGOCLAW_REDIS_DSN", &c.Database.RedisDSN)

	// Deprecation warning for ARGOCLAW_MODE (removed — PostgreSQL is always active)
	if v := os.Getenv("ARGOCLAW_MODE"); v != "" {
		slog.Warn("ARGOCLAW_MODE is deprecated; managed mode is now the only mode", "value", v)
	}

	// Telemetry
	envStr("ARGOCLAW_TELEMETRY_ENDPOINT", &c.Telemetry.Endpoint)
	envStr("ARGOCLAW_TELEMETRY_PROTOCOL", &c.Telemetry.Protocol)
	envStr("ARGOCLAW_TELEMETRY_SERVICE_NAME", &c.Telemetry.ServiceName)
	if v := os.Getenv("ARGOCLAW_TELEMETRY_ENABLED"); v != "" {
		c.Telemetry.Enabled = v == "true" || v == "1"
	}
	if v := os.Getenv("ARGOCLAW_TELEMETRY_INSECURE"); v != "" {
		c.Telemetry.Insecure = v == "true" || v == "1"
	}

	// Owner IDs from env (comma-separated)
	if v := os.Getenv("ARGOCLAW_OWNER_IDS"); v != "" {
		c.Gateway.OwnerIDs = strings.Split(v, ",")
	}

	// Tailscale (tsnet)
	envStr("ARGOCLAW_TSNET_HOSTNAME", &c.Tailscale.Hostname)
	envStr("ARGOCLAW_TSNET_AUTH_KEY", &c.Tailscale.AuthKey)
	envStr("ARGOCLAW_TSNET_DIR", &c.Tailscale.StateDir)
...
// ResolvedDataDirFromEnv returns the data dir from ARGOCLAW_DATA_DIR env or default.
// Use this in packages that don't have access to a Config instance.
func ResolvedDataDirFromEnv() string {
	if v := os.Getenv("ARGOCLAW_DATA_DIR"); v != "" {
		return ExpandHome(v)
	}
	return ExpandHome("~/.argoclaw/data")
}
...
// Falls back to "ArgoClaw" if not configured.
